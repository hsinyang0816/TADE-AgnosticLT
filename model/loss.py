...
class LDAMLoss(nn.Module):
    def __init__(self, cls_num_list=None, max_m=0.5, s=30, reweight_epoch=-1):
        ...
        if cls_num_list is None:
            ...
            m_list = torch.tensor(
                m_list, dtype=torch.float, requires_grad=False)
            self.m_list = m_list
            assert s > 0
            self.s = s
            if reweight_epoch != -1:
    ...
    def to(self, device):
        ...
        if self.per_cls_weights_enabled is not None:
            self.per_cls_weights_enabled = self.per_cls_weights_enabled.to(
                device)
...
class RIDELoss(nn.Module):
        ...
        if cls_num_list is None:
            ...
            if reweight_epoch != -1:
                ...
                per_cls_weights = per_cls_weights / \
                    np.sum(per_cls_weights) * len(cls_num_list)
                self.per_cls_weights_enabled = torch.tensor(
                    per_cls_weights, dtype=torch.float, requires_grad=False)   # 这个是logits时算CE loss的weight
            else:
                self.per_cls_weights_enabled = None
            ...
            assert np.all(per_cls_weights >
                          0), "reweight factor is too large: out of bounds"
            # save diversity per_cls_weights
            self.per_cls_weights_enabled_diversity = torch.tensor(
                per_cls_weights, dtype=torch.float, requires_grad=False).cuda()  # 这个是logits时算diversity loss的weight
    ...
    def to(self, device):
        ...
        if self.per_cls_weights_enabled_diversity is not None:
            self.per_cls_weights_enabled_diversity = self.per_cls_weights_enabled_diversity.to(
                device)
    ...
    def forward(self, output_logits, target, extra_info=None):
        ...
        for logits_item in extra_info['logits']:
            ...
            if self.m_list is None:
                loss += self.base_loss_factor * \
                    self.base_loss(ride_loss_logits, target)
            else:
                final_output = self.get_final_output(ride_loss_logits, target)
                loss += self.base_loss_factor * \
                    self.base_loss(final_output, target,
            ...
            output_dist = F.log_softmax(
                logits_item / diversity_temperature, dim=1)
            with torch.no_grad():
            ...
            loss += self.additional_diversity_factor * temperature_mean * temperature_mean * \
...
class DiverseExpertLoss(nn.Module):
    ...
    def forward(self, output_logits, target, epoch=-1, extra_info=None):
        ...
        expert3_logits = expert3_logits + \
            torch.log(self.prior + 1e-9) - self.tau * \
            torch.log(inverse_prior + 1e-9)
